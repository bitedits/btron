...
defmodule BtronBook.Compiler do
  ...
  defp scan_stream(html, acc) do
    ...
    case Regex.run(tag_regex, html, return: :index) do
      [{offset, len}] ->
        acc =
          if offset > 0 do
            lead = binary_part(html, 0, offset)
            if md = parse_markdown_table(lead) do
              [{:table, md} | acc]
            else
              acc
            end
      ...
      nil ->
        if md = parse_markdown_table(html) do
          Enum.reverse([{:table, md} | acc])
        else
          Enum.reverse(acc)
        end
    end
  ...
  def wrap_cell_text(text, max_len) when is_binary(text) and max_len > 0 do
    ...
    if words == [] do
      ...
      # Ensure no single token exceeds max_len
      split_words =
  ...
  def format_table_ascii(rows, max_table_width) when is_list(rows) and length(rows) > 0 do
    ...
    if col_count == 0 do
      ...
      # 1. Natural width & max single word length of each column
      natural_widths =
      ...
      # 4. Box Drawing Borders (Seamless 8x16 Line Glyphs)
      top_border =
      ...
      # 5. Format Rows with Multi-Line Wrapping
