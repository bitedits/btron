...
.BASE = 0L
	...
	SPNL+107	epnl_pushtray	-- トレーへ複写/移動できない
	SPNL+108	epnl_poptray	-- トレーから複写/移動できない
	SPNL+109	epnl_paste	-- ウィンドウへ複写/移動できない
	SPNL+110	epnl_tmptray	-- 一時トレーに格納できない
	SPNL+111	epnl_fsnexec	-- 付箋起動は禁止
	SPNL+112	epnl_loadskip	-- TAD の読み飛ばしあり
	SPNL+113	epnl_vobjskip	-- 仮身の読み飛ばしあり
	SPNL+114	epnl_read	-- 実身からの読み込みに失敗
	SPNL+115	epnl_write	-- 実身への書き込みに失敗
	SPNL+116	epnl_newobj	-- 実身生成失敗
	SPNL+117	epnl_newvobj	-- 仮身複製失敗
	SPNL+118	epnl_undomake	-- 取り消し生成失敗

	-- その他のパネル
	SPNL+200	opnl_tidy	-- 整頓設定パネル
...
.str_vobjskip:	"仮身の数が制限を超えましたので残りを読み飛ばしました。\0"
.epnl_vobjskip:	TEXT+pMID+pERR			str_vobjskip
		TEXT+pMID			str_loadskip1
		TEXT+pMID			str_loadskip2
		PART+pMID+pDEF+pEND		sw_cnfm
...
.str_newvobj:	"仮身／実身の複写ができません。\0"
.epnl_newvobj:	TEXT+pMID+pERR			str_newvobj
		TEXT+pMID+pINE			err_dmy
		PART+pMID+pDEF+pEND		sw_cnfm

.str_undomake:	"処理中に障害が発生しましたので、取り消しはできません。\0"
.epnl_undomake:	TEXT+pMID+pERR			str_undomake
		TEXT+pMID+pINE			err_dmy
		PART+pMID+pDEF+pEND		sw_cnfm
...
.ws_tidy_h:	WS_PARTS+P_NOSEL
		{0+CU	0	CHSSTD*12+8	CHSSTD+2*2+8}
		0L	OFFSET:L+4
		MC_STR	"先頭の左側にそろえる"
		MC_STR	"先頭の右側にそろえる"
		0
.ws_tidy_v:	WS_PARTS+P_NOSEL
		{0+CU	0	CHSSTD*12+8	CHSSTD+2*2+8}
		0L	OFFSET:L+4
		MC_STR	"間隔を詰める"
		MC_STR	"先頭の間隔にそろえる"
		0
.ws_tidy_l:	WS_PARTS+P_NOSEL
		{0+CU	0	CHSSTD*12+8	CHSSTD+2*5+8}
		0L	OFFSET:L+4
		MC_STR	"先頭の長さにそろえる"
		MC_STR	"全体を表示する長さ"
		MC_STR	"名前のみ表示する長さ"
...
.opnl_tidy:
		-- 横
		Vpos				6
		TEXT+pFIL+pNXT			str_tidy_h
		PART+pIN3			ws_tidy_h

		-- 縦
		Vadj				-4
		Vpos				6
		TEXT+pFIL+pNXT			str_tidy_v
		PART+pIN4			ws_tidy_v
		...
		Vadj				-100
		Vpos				6
		TEXT+pFIL+pNXT			str_tidy_l
		PART+pIN5			ws_tidy_l

		-- [取り消し]/[実行]
		PART+pIN1+pNXT			sw_cancel
		PART+pIN2+pRIT+pDEF+pEND	sw_do
