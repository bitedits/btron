...
// 広域変数
EXPORT	I_DLED_APPL*	appl = NULL;	// application 基幹
...
LOCAL	ERR	ex_main(const MESSAGE* msg)
{
	...
	if (opendatabox(&lnk, DBOX::I_DLED_DTYP,DBOX::I_DLED_DNUM,1) < ER_OK) {
		throw EXCEPT_INITERR(I_DLEDERR::DBOXOPEN);
	}
	...
	switch (msg->msg_type) {
		case EXECREQ:		// 仮身のオープン起動
			...
			if (((M_EXECREQ*)msg)->mode & 0x0002) {
				...
				// 小物以外の起動方法の場合
				appl = new I_DLED_EXREQ((M_EXECREQ*)msg);
			}
			break;   
		case FUSENREQ:		// 付箋のオープン起動
			DPRINT(("execution : FUSENREQ\n"));
			oend_prc(((M_FUSENREQ*)msg)->vid, NULL, 0);
			throw EXCEPT_EXECMSG(I_DLEDERR::FSNEXEC);
			break;
		case DISPREQ:		// 開いた仮身の表示起動
			DPRINT(("execution : DISPREQ\n"));
			appl = new I_DLED_DSPREQ((M_DISPREQ*)msg);
			break;
		case PASTEREQ:		// データ貼り込み起動
			DPRINT(("execution : PASTEREQ\n"));
			appl = new I_DLED_PSTREQ((M_PASTEREQ*)msg);
			break;
		case TADREQ:		// 開いた仮身の TAD データ作成起動
					// oend_req() せずに error 終了すると、
					// omgr が DISPREQ で再起動してくる
			DPRINT(("execution : TADREQ\n"));
			throw EXCEPT_EXECMSG(I_DLEDERR::NOEXEC);
			break;
...
extern "C" W MAIN(MESSAGE* msg)
{
	...
	try {
		...
		er = err.get_err();
		DPRINT(("%s : %d\n", err.what(), er >> 16));
		if (er == I_DLEDERR::FSNEXEC) {
			// 付箋起動禁止
			errpanel(DBOX::EPNL_FSNEXEC, 0);
		} else if (er == I_DLEDERR::NOEXEC) {
			// 他(panel は出さない)
			er = ER_NOSPT;
		} else {
			// 他
