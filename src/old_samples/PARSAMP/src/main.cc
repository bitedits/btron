...
// 広域変数
EXPORT	PARSAMP_APPL*	appl = NULL;	// application 基幹
...
LOCAL	ERR	ex_main(const MESSAGE* msg)
{
	...
	if (opendatabox(&lnk, DBOX::PARSAMP_DTYP, DBOX::PARSAMP_DNUM, 1) < ER_OK) {                                   
		throw EXCEPT_INITERR(PARSAMPERR::DBOXOPEN);
	}
	...
	switch (msg->msg_type) {
		case EXECREQ:		// 仮身のオープン起動
			...
			if (((M_EXECREQ*)msg)->mode & 0x0002) {
				// 小物起動の場合
				appl = new PARSAMP_EXREQ((M_EXECREQ*)msg);
			} else {
				// 小物以外の起動方法の場合
		...
		case FUSENREQ:		// 付箋のオープン起動
			...
			appl = new PARSAMP_FSNREQ((M_FUSENREQ*)msg);
			break;
